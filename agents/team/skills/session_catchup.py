...
class TeamSessionCatchupSkill:
    ...
    def execute(self, request: TeamCatchupRequest) -> str:
        ...
        system_prompt = (
            ...
            "   - If the query asks a SPECIFIC question (e.g., 'what assignments were given?', 'what decisions were made?', 'what did we cover on Qdrant?'), give a FOCUSED direct answer targeting exactly that — do NOT produce all 6 catch-up schema sections when only one is relevant.\n"
            "   - If the query asks for a FULL CATCH-UP or COMPLETE SESSION SUMMARY (e.g., 'I missed the session, catch me up', 'what happened?', 'full recap'), THEN use the complete output schema.\n"
            "   - The answer length and structure must be PROPORTIONAL to the question. A narrow question → a focused answer. A full catch-up request → the full schema.\n"
