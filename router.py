...
def route_request(user_prompt: str, user_role: str = "auto", target_member: str = "") -> str:
    ...
    # Intent Override: Evaluation & Performance requests ALWAYS route to Mentor Agent
    if any(word in prompt_lower for word in ["evaluate", "performance", "score", "quiz"]):
        target = target_member if target_member else (user_role.capitalize() if role_lower in ["himaya", "ganesh", "dakshinya"] else "")
